...
def cli(
    ...
    --remove-metadata is explicitly supplied.
    """
    ...
    try:
        summary = main(directory, output, remove_metadata)
    except OSError as exc:
        raise click.ClickException(f"could not create report: {exc}") from exc

    if not quiet:
        _print_summary(Console(highlight=False), output, summary, remove_metadata)
